...
consumer so far has its own storage shape (fashion-monitor uses SQLite with
...
because one real project needs it today (fashion-monitor's New Balance
...
- **`poll`** — implemented. Fetches one feed URL over HTTPS (via
  ...
  Extracted from fashion-monitor's `apps/cli/src/pipeline/bulletin-poll.ts`
